...
do
ip=$((i+1))
CID=$(docker run -d --restart always --privileged --dns 8.8.8.8 --name server$i -h server$i --publish-all=true -d  --net=es_bridge --ip 172.18.0.$ip -e PGHOST='db_server' -e PGUSER='postgres' -i -t elastic/server)
